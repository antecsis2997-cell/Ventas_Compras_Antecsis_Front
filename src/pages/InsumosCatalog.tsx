...
export default function InsumosCatalog() {
  ...
  const loadInsumos = useCallback(async (pageNum: number = 0) => {
    setLoading(true);
    try {
      const res = await api.get("/api/insumos", { params: { page: pageNum, size: 10 } });
      const data = res.data;
  ...
  useEffect(() => {
    api
      .get("/api/categorias", { params: { page: 0, size: 500 } })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        precio,
        // Por ahora el insumo no requiere precio de compra en tu UI; puede quedar null
        precioCompra: null,
        stock: 0,
        ...
        // Asegura que sea insumo
        esInsumo: true,
      };

      if (editingId) {
        // mantener stock si ya existía
        const existing = insumos.find((p) => p.id === editingId);
        await api.put(`/api/insumos/${editingId}`, { ...payload, stock: existing?.stock ?? 0 });
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("¿Eliminar este insumo? (se hará eliminación lógica)")) return;
    try {
    ...
    <>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Alta y mantenimiento de materia prima</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                ...
                <th className="px-5 py-3 text-left font-medium text-muted-foreground">Insumo</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="px-5 py-4 text-center text-muted-foreground">
                    Cargando...
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                ...
                filtered.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-foreground">
                      {formatMoney(Number(p.precio ?? 0), (p.moneda as "PEN" | "USD") ?? "PEN")}
                    </td>
      ...
      <div className="flex justify-between items-center mt-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" disabled={page === 0} onClick={() => loadInsumos(page - 1)}>
            Anterior
          </Button>
          <Button
            variant="outline"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, stockMinimoAlerta: e.target.value }))}
              />
            </div>
